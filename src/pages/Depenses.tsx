...
import {
  ...
  useUpdateDepense,
  useDeleteDepense,
} from "../hooks/queries/useDepenses";
...
export default function Depenses() {
  const [period, setPeriod] = useState(PERIODES[new Date().getMonth()]);
  ...
  const {
    ...
    isLoading,
    error,
  } = useDepenses(period || undefined);
  const { data: allDepenses = [] } = useDepenses();
  ...
  const recap = useMemo(() => {
    const rows = PERIODES.map((p) => {
      const moisDeps = allDepenses.filter((d) => d.periode === p);
      const entrees = moisDeps
        .filter((d) => d.type === "ENTREE BANQUE")
    ...
    const totS = rows.reduce((s, r) => s + r.sorties, 0);
    return { rows, totE, totS };
  }, [allDepenses]);

  const filtered = depenses.filter(
    (d) =>
      (!filType || d.type === filType) && (!filCat || d.categorie === filCat),
  );
  ...
  return (
    <div className="pg">
                ...
                (filtered.length === 0 ? (
                  ...
                  filtered.map((d) => (
                    <tr key={d.id}>
                      ...
                      <td>
                        <span
                          className={`bdg ${d.type === "ENTREE BANQUE" ? "bg" : "br"}`}
                          style={{ fontSize: 9.5 }}
                        >
                      ...
                      <td
                        className={`tnum fw7 ${d.type === "ENTREE BANQUE" ? "tc" : "rc"}`}
                      >
                        {fmt(d.montant)}
                      </td>
                      <td
      ...
      <div className="card" style={{ marginBottom: 14 }}>
        <div className="ch">
          <h3>Récapitulatif annuel {new Date().getFullYear()}</h3>
        </div>
        <div className="tw">
          <table>
            <thead>
              <tr>
                ...
                {MF_SHORT.map((m, i) => (
                  <th
                    ...
                    style={{
                      fontSize: 10.5,
                      fontWeight: PERIODES[i] === period ? 700 : 400,
                      color: PERIODES[i] === period ? "var(--B)" : undefined,
                    }}
                  >
                    {m}
                  </th>
                ))}
                <th className="tr" style={{ fontSize: 10.5 }}>
                  TOTAL
                </th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td
                  style={{ fontSize: 11, color: "var(--G)", fontWeight: 600 }}
                >
                  ↑ Entrées banque
                </td>
                {recap.rows.map((r, i) => (
                  <td
                    ...
                    style={{
                      fontSize: 11,
                      color: r.entrees ? "var(--G)" : "var(--tx3)",
                    }}
                  >
                    {r.entrees ? fmt(r.entrees) : "—"}
                  </td>
                ))}
                <td className="tnum fw7" style={{ color: "var(--G)" }}>
                  {fmt(recap.totE)}
                </td>
              </tr>
              <tr>
                <td
                  style={{ fontSize: 11, color: "var(--R)", fontWeight: 600 }}
                >
                  ↓ Sorties banque
                </td>
                {recap.rows.map((r, i) => (
                  <td
                    ...
                    style={{
                      fontSize: 11,
                      color: r.sorties ? "var(--R)" : "var(--tx3)",
                    }}
                  >
                    {r.sorties ? fmt(r.sorties) : "—"}
                  </td>
                ))}
                <td className="tnum fw7" style={{ color: "var(--R)" }}>
                  {fmt(recap.totS)}
                </td>
              </tr>
              <tr style={{ borderTop: "1px solid var(--bor)" }}>
                <td style={{ fontSize: 11, fontWeight: 600 }}>Solde net</td>
                {recap.rows.map((r, i) => {
                  const s = r.entrees - r.sorties;
                  return (
                    <td
                      key={i}
                      className="tnum"
                ...
                <td
                  ...
                  style={{
                    color:
                      recap.totE - recap.totS >= 0 ? "var(--G)" : "var(--R)",
                  }}
                >
                  {fmt(recap.totE - recap.totS)}
                </td>
              </tr>
            </tbody>
          </table>
      ...
      >
        <div className="fg">
          <Field
            ...
            onChange={(e) =>
              form.set("type", e.target.value as DepenseInput["type"])
            }
          >
          ...
          <Field
            ...
            onChange={(e) =>
              form.set("nature", e.target.value as DepenseInput["nature"])
            }
          >
          ...
          <Field
            ...
            placeholder="Ex : Virement salaires mars"
          />
          <Field
            label="Intitulé"
            value={form.values.int}
            onChange={(e) => form.set("int", e.target.value)}
          ...
          <Field
            label="Montant (FCFA) *"
            type="number"
            value={form.values.mnt}
            error={form.errors.mnt}
            onChange={(e) => form.set("mnt", +e.target.value)}
          />
          <Field
            label="Frais de transfert"
            type="number"
            value={form.values.ft}
