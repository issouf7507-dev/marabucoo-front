...
import {
  ...
  useUpdatePC,
  useDeletePC,
} from "../hooks/queries/usePetiteCaisse";
...
export default function PetiteCaisse() {
  ...
  const soldeParCaisse = ["PRINCIPALE", "BEN SOUDA"].map((c) => {
    ...
    return {
      caisse: c,
      solde: rows.length > 0 ? rows[rows.length - 1].solde : 0,
    };
  });
  const soldeTotal = soldeParCaisse.reduce((s, x) => s + x.solde, 0);

  const totalEntrees = all
    .filter((p) => p.type === "entree")
    .reduce((s, p) => s + p.entree, 0);
  const totalSorties = all
    .filter((p) => p.type === "sortie")
    .reduce((s, p) => s + p.sortie + p.penalite, 0);
    ...
    const datasets = [
      {
        ...
        data: PERIODES.map((per) =>
          all
            .filter((x) => x.caisse === "PRINCIPALE" && x.periode === per)
            .reduce((s, x) => s + x.entree, 0),
        ),
      ...
      },
      {
        ...
        data: PERIODES.map((per) =>
          all
            .filter((x) => x.caisse === "PRINCIPALE" && x.periode === per)
            .reduce((s, x) => s + x.sortie + x.penalite, 0),
        ),
      ...
      },
      {
        ...
        data: PERIODES.map((per) =>
          all
            .filter((x) => x.caisse === "BEN SOUDA" && x.periode === per)
            .reduce((s, x) => s + x.entree, 0),
        ),
      ...
      chartInst.current = new Chart(chartRef.current, {
        ...
        options: {
          ...
          scales: {
            y: {
              ticks: {
                callback: (v: unknown) => Math.round(Number(v) / 1000) + "K",
                font: { size: 10 },
              },
    ...
    <div className="pg">
      <div className="ph">
        <div className="ph-l">
          <h1>Petite caisse</h1>
          <p>
            Gestion séparée — numérotation automatique pour archivage physique
          </p>
        </div>
        <div className="ph-r">
      ...
      <div className="tabs">
        <button
          className={`tab${tab === "registre" ? " on" : ""}`}
          onClick={() => setTab("registre")}
        >
          Registre
        </button>
        <button
          className={`tab${tab === "factures" ? " on" : ""}`}
        ...
        >
          ...
          Factures
          {factures.length > 0 && (
            <span className="bdg bg" style={{ marginLeft: 6, fontSize: 9 }}>
              {factures.length}
            </span>
          )}
        </button>
      ...
      {tab === "registre" && (
        <>
            ...
            <div className="ch fb">
              ...
              <div>
                <select
                  className="sel"
                  value={filCaisse}
                  onChange={(e) => setFilCaisse(e.target.value)}
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th>Désignation</th>
                    <th>Prestataire</th>
                    <th className="tr">Entrée</th>
                    <th className="tr">Sortie</th>
                    <th className="tr">Pénalité</th>
                    ...
                    (filtered.length === 0 ? (
                      ...
                      filtered.map((p) => (
                        <tr key={p.id}>
                          ...
                          <td>
                            <span className="bdg bb">{p.caisse}</span>
                          </td>
                          <td>
                            <span
                              className={`bdg ${p.type === "entree" ? "bg" : "br"}`}
                            >
                          ...
                          <td
                            ...
                            style={{
                              color: "var(--G)",
                              fontWeight: p.entree > 0 ? 700 : undefined,
                            }}
                          >
                          ...
                          <td
                            ...
                            style={{
                              color: "var(--R)",
                              fontWeight: p.sortie > 0 ? 700 : undefined,
                            }}
                          >
                            {p.sortie > 0 ? `-${fmt(p.sortie)}` : "—"}
                          </td>
                          <td className="tnum" style={{ color: "var(--tx3)" }}>
                            {p.penalite > 0 ? fmt(p.penalite) : "—"}
                          </td>
                          <td
                            className="tnum fw7"
                            style={{
                              color: p.solde >= 0 ? "var(--G)" : "var(--R)",
                            }}
                          >
                            {fmt(p.solde)}
                          </td>
          ...
          <div className="card">
            <div className="ch">
              <h3>Flux petite caisse par mois et par caisse</h3>
            </div>
            {isLoading || error ? (
              <div className="cb">
                <QueryCard isLoading={isLoading} error={error} />
              </div>
            ) : (
              <>
                <div className="cb">
                ...
                <div className="tw" style={{ marginTop: 16 }}>
                  <table>
                    <thead>
                      <tr>
                        <th>Mois</th>
                        <th className="tr" style={{ color: "var(--G)" }}>
                          PRINCIPALE Entrées
                        </th>
                        <th className="tr" style={{ color: "var(--R)" }}>
                          PRINCIPALE Sorties
                        </th>
                        <th className="tr" style={{ color: "#1A4E8A" }}>
                          BEN SOUDA Entrées
                        </th>
                        <th className="tr" style={{ color: "#ea580c" }}>
                          BEN SOUDA Sorties
                        </th>
                    ...
                    <tbody>
                      {PERIODES.map((per) => {
                        const pE = all
                          .filter(
                            (x) =>
                              x.caisse === "PRINCIPALE" && x.periode === per,
                          )
                          .reduce((s, x) => s + x.entree, 0);
                        const pS = all
                          .filter(
                        ...
                        const bE = all
                          .filter(
                            (x) =>
                              x.caisse === "BEN SOUDA" && x.periode === per,
                          )
                          .reduce((s, x) => s + x.entree, 0);
                        const bS = all
                          .filter(
                        ...
                        return (
                          <tr key={per}>
                            ...
                            <td className="tnum" style={{ color: "var(--G)" }}>
                              {pE > 0 ? `+${fmt(pE)}` : "—"}
                            </td>
                            <td className="tnum" style={{ color: "var(--R)" }}>
                              {pS > 0 ? `-${fmt(pS)}` : "—"}
                            </td>
                            <td className="tnum" style={{ color: "#1A4E8A" }}>
                              {bE > 0 ? `+${fmt(bE)}` : "—"}
                            </td>
                            <td className="tnum" style={{ color: "#ea580c" }}>
                              {bS > 0 ? `-${fmt(bS)}` : "—"}
                            </td>
                            <td
                              className="tnum fw7"
                  ...
                  {factures.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="empty">
                        Aucune pièce justificative enregistrée — ajoutez une
                        référence facture lors de la saisie
                      </td>
                    </tr>
                  ) : (
                    factures.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td>
                          <span className="bdg bn">{p.caisse}</span>
                        </td>
                        <td style={{ fontSize: 11.5 }}>{p.designation}</td>
                        <td style={{ fontSize: 11, color: "var(--tx3)" }}>
                          {p.prestataire ?? "—"}
      ...
      >
        <div className="fg">
          <Field
            as="select"
            label="Caisse *"
            value={form.values.caisse}
            error={form.errors.caisse}
            onChange={(e) =>
              form.set("caisse", e.target.value as PetiteCaisseInput["caisse"])
            }
          >
            <option value="PRINCIPALE">Caisse principale</option>
            <option value="BEN SOUDA">Caisse Ben Souda</option>
          ...
          <Field
            ...
            onChange={(e) =>
              form.set("nature", e.target.value as PetiteCaisseInput["nature"])
            }
          >
          ...
          <Field
            ...
            error={form.errors.des}
            onChange={(e) => form.set("des", e.target.value)}
            placeholder="Ex : Approvisionnement carburant"
          />
          <Field
            as="select"
            label="Catégorie"
          ...
          <Field
            label="Prestataire"
            value={form.values.prest}
            onChange={(e) => form.set("prest", e.target.value)}
          />
          {isEntree ? (
            <Field
              label="Montant entrée (FCFA) *"
              type="number"
              value={form.values.entree}
              error={form.errors.entree}
              onChange={(e) => form.set("entree", +e.target.value)}
            />
          ) : (
            <>
              <Field
                label="Montant sortie (FCFA) *"
                type="number"
                value={form.values.sortie}
                error={form.errors.entree}
                onChange={(e) => form.set("sortie", +e.target.value)}
              />
              <Field
                label="Pénalité (FCFA)"
                type="number"
                value={form.values.pen}
